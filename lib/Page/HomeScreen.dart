...
import 'package:order_processing_app/page/CreateOderScreen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
        ...
        child: ListView(
          children:  [
             const UserAccountsDrawerHeader(
                ...
                accountName: Text('Беляев Артём'),
                accountEmail: Text("artem_belyaev_2014@list.ru")),
            ...
            GestureDetector(
              ...
              child: const ListTile(
                title: Text("Создать новую заявку"),
      ...
      body: Container(
        child: StreamBuilder(
          stream: FirebaseFirestore.instance.collection('Заявки').where('userId', isEqualTo: user!.uid).snapshots(),
                          ...
                          trailing: CircleAvatar(child: InkWell(
                              onTap: (){
                                Get.defaultDialog(
                                  ...
                                  title: "Изменить статус заявки?", content: const Text(""),
                                  onCancel: (){},
                                  onConfirm: () async {
                                    EasyLoading.show();
                                    await FirebaseFirestore.instance.collection('Заявки').doc(docId).update({'status': 'Исполнено'},);
                                    Get.back();
                                    EasyLoading.dismiss();
                                  },
                                );
